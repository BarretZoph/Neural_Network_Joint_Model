...
class minibatcher:
    ...
    def prep_train(self):
        ...
        self.current_index = 0
    def unint_batch(self,batch):
        minibatch_size = batch.shape[0]
        src_p_tgt = batch.shape[1]
    	assert src_p_tgt==(self.src_window+self.tgt_window+1),"Error not correct data format"
        arr = []
        for i in range(0,minibatch_size):
            tmp = []
            for j in range(0,self.src_window):
                tmp.append(self.rev_src_mapping[batch[i,j]])
            for j in range(self.src_window,self.src_window+self.tgt_window+1):
                tmp.append(self.rev_tgt_mapping[batch[i,j]])
            arr.append(tmp)
    ...
    def load_mapping(self,mapping_file):
    	...
    	self.src_mapping = {}
    	self.tgt_mapping = {}
    	self.rev_src_mapping = {}
    	self.rev_tgt_mapping = {}
    	for line in codecs.open(mapping_file,'r','utf-8'):
    		...
    		if source:
    			...
    			self.rev_src_mapping[int(line[0])] = line[1]
    		else:
    			self.tgt_mapping[line[1]] = int(line[0])
    			self.rev_tgt_mapping[int(line[0])] = line[1]
